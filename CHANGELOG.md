...
- The **Figma** connector now shows its real multi-color brand mark (drawn from
...
- **The agent and its grounding folder moved to the left rail**, grouped with the
  ...
  is now the canvas tools, zoom, and a board Copy button.
- Agent chat **tool calls collapse to a single line** — truncated, with the full
  text on hover — so a burst of edits stays scannable.
- The left **rail gutter tightened from 9% to 6%**, so the rail reads as attached
  to the board rather than marooned at the screen edge.
