...
enum CanvasMCP {
  ...
  private static let opForTool: [String: String] = [
    ...
    "add_sticky": "add_sticky", "set_sticky": "set_sticky", "add_checklist": "add_checklist", "set_checklist": "set_checklist", "toggle_checklist_item": "toggle_checklist_item", "add_table": "add_table", "set_table": "set_table",
  ...
  private static let toolSpecs: [[String: Any]] = [
    ...
    tool("add_shape", "Add a shape sized by a bounding box. Omit x/y to auto-place.",
         ...
         ["kind", "w", "h"]),
    tool("add_sticky", "Add a sticky note with a separate title and body. Omit x/y to auto-place.",
         ["title": str("Note title"), "text": str("Note body"), "x": num("Optional top-left x"), "y": num("Optional top-left y")], ["text"]),
    tool("set_sticky", "Replace a sticky note's title and body.",
         ["id": str("Sticky note id"), "title": str("Note title"), "text": str("Note body")], ["id", "title", "text"]),
    tool("add_checklist", "Add a tappable checklist. Omit x/y to auto-place.",
         ["items": ["type": "array", "items": ["type": "object", "properties": ["text": str("Task"), "checked": ["type": "boolean"]], "required": ["text"]]],
          "x": num("Optional top-left x"), "y": num("Optional top-left y")], ["items"]),
    tool("toggle_checklist_item", "Toggle one checklist item by zero-based index.",
         ["id": str("Checklist node id"), "index": num("Zero-based item index")], ["id", "index"]),
    tool("set_checklist", "Replace all items in an existing checklist.",
         ["id": str("Checklist node id"), "items": ["type": "array", "items": ["type": "object", "properties": ["text": str("Task"), "checked": ["type": "boolean"]], "required": ["text"]]]], ["id", "items"]),
    tool("add_table", "Add a simple table. Every row should match the column count.",
         ["columns": ["type": "array", "items": ["type": "string"]],
          "rows": ["type": "array", "items": ["type": "array", "items": ["type": "string"]]],
          "x": num("Optional top-left x"), "y": num("Optional top-left y")], ["columns", "rows"]),
    tool("set_table", "Replace the columns and rows of an existing table.",
         ["id": str("Table node id"), "columns": ["type": "array", "items": ["type": "string"]],
          "rows": ["type": "array", "items": ["type": "array", "items": ["type": "string"]]]], ["id", "columns", "rows"]),
